(() => {
  ...
  function refresh(){
    const s=stats(answers,STATIONS);$('score').textContent=s.score;$('progress-text').textContent=`${s.done} / ${total} esplorati`;$('progress-bar').style.width=`${s.done/total*100}%`;$('found-count').textContent=`${s.found} / ${riskTotal} potenziali focolai`;
    $('journal-list').innerHTML=STATIONS.map(item=>{const done=Object.hasOwn(answers,item.id),correct=done&&answers[item.id]===item.risk;return `<button class="journal-row ${done?'done':''} ${done&&!correct?'wrong':''}" data-review="${item.id}" ${done?'':'disabled'}><span class="journal-number">${done?(correct?'✓':'!'):item.code}</span><span>${done?item.name:`Stazione ${item.code}`}<small>${done?(item.risk?'Potenziale focolaio':'Non è un potenziale focolaio'):'Ancora da esplorare'}</small></span>${done?`<span class="row-score">${correct?'+100':'0'} PT</span>`:''}</button>`;}).join('');
  ...
  function renderStation(){
    ...
    $('station-content').innerHTML=`<div class="eyebrow">STAZIONE ${s.code} / ${total}</div><span class="station-icon" aria-hidden="true">${symbols[s.icon]}</span><h2 id="station-title">${s.name}</h2><p>${s.scene}</p>${done?`<div class="answer-banner ${correct?'':'incorrect'}">${correct?'✓ Esatto! +100 punti':'Un indizio per la prossima volta · 0 punti'}</div><h3>${s.risk?'È un potenziale focolaio larvale.':'Non è un potenziale focolaio, nelle condizioni descritte.'}</h3><p>${s.fact}</p><div class="action-box"><strong>IL GESTO CHE CONTA</strong><p>${s.action}</p></div><button class="button primary full" id="continue-hunt">${stats(answers,STATIONS).complete?'Scopri il risultato':'Continua la caccia'} →</button>`:`<h3>È un potenziale focolaio larvale?</h3><div class="choice-buttons"><button class="button primary" data-answer="true">Sì, è un potenziale focolaio</button><button class="button secondary" data-answer="false">No, non lo è</button></div><p class="question-hint">+100 punti per ogni risposta corretta. Osserva le condizioni descritte.</p>`}`;
  }
  ...
  function finish(){
    ...
    $('results').innerHTML=`<div class="result-mark" aria-hidden="true">✳</div><div class="eyebrow">${mode==='demo'?'DEMO COMPLETATA':'MISSIONE COMPLETATA'}</div><h1>${s.correct===total?'Occhio da ricercatore!':'Ora sai dove guardare.'}</h1><p>Hai esplorato tutti gli oggetti.<br>La prossima scoperta può cominciare dal tuo balcone.</p><div class="result-stats"><div><strong>${s.score}</strong><span>punti su ${total*100}</span></div><div><strong>${s.correct} / ${total}</strong><span>risposte corrette</span></div><div><strong>${s.found} / ${riskTotal}</strong><span>potenziali focolai riconosciuti</span></div></div><div class="takeaway"><h2>Una piccola abitudine. Ogni settimana.</h2><p>Controlla i contenitori all’aperto: svuota e pulisci quelli che trattengono acqua, capovolgili o coprili. Per prevenire le zanzare, comincia dai ristagni.</p></div><details class="result-review"><summary>Rivedi tutte le scoperte</summary>${STATIONS.map(item=>`<article><h3>${item.code} · ${item.name} — ${item.risk?'potenziale focolaio':'non è un potenziale focolaio'} ${answers[item.id]===item.risk?'✓':'· da ripassare'}</h3><p>${item.fact} ${item.action}</p></article>`).join('')}</details><div class="choice-buttons"><button id="new-game" class="button primary">Ricomincia la caccia ↗</button><button id="result-home" class="button secondary">Torna all’inizio</button></div>${mode==='demo'?'<p class="question-hint">Questa era una prova. I progressi della caccia allo stand sono separati.</p>':''}`;
